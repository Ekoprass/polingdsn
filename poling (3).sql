...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 22, 2018 at 04:49 AM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rank`  AS  select substr(`polingdsn`.`id_kelas`,1,6) AS `tahun`,`polingdsn`.`id_dosen` AS `id_dosen`,`polingdsn`.`nama_dosen` AS `nama_dosen`,`polingdsn`.`nilai` AS `nilai`,find_in_set(`polingdsn`.`nilai`,(select group_concat(distinct `polingdsn`.`nilai` order by `polingdsn`.`nilai` DESC separator ',') from `polingdsn`)) AS `rank` from `polingdsn` group by `polingdsn`.`id_dosen` order by `polingdsn`.`nama_dosen` ;
