					...
					<th>
						...
						<?php
							...
							if($cekkelas>0){ 
									if($k==04){?>
										<a href="<?php echo site_url('kelas/naik/'.$kelas_siswa['id_kelas']);?>">
											<button disabled class="btn btn-success btn-lg"><i class="glyphicon glyphicon-eject"></i>  Naik Kelas  <?php echo $id;?></button>
										</a>
								<?php }else{ ?>
									<a href="<?php echo site_url('kelas/naik/'.$kelas_siswa['id_kelas']);?>">
										<button disabled class="btn btn-success btn-lg"><i class="glyphicon glyphicon-eject"></i>  Naik Kelas  <?php echo $id;?></button>
							...
							<a href="<?php echo site_url('kelas/tambahnaik/'.$id.'/'.$kelas_siswa['id_kelas']);?>">
								<button disabled class="btn btn-success btn-lg"><i class="glyphicon glyphicon-eject"></i>  Naik Kelas Dan Tambah  <?php echo $id;?></button>
							</a>
						<?php } 
						?>
					</th>
					...
					<table class="table table-hover table-responsive table-bordered">
						...
						<form class="form-horizontal" action="<?php echo site_url('kelas/tambah_siswa_proses');?>" method="post" enctype="multipart/form-data">
							<tr>
								...
								<td colspan=4>
									<select disabled required name="nim" class="form-control">
								...
								<td  align=center>
									<button disabled class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i> Tambah</button>
								</td>
							</tr>
						</form>
